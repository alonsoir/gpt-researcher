...
class GPTResearcher:
    ...
    async def conduct_research(self):
        ...
        if not (self.agent and self.role):
            self.agent, self.role = await choose_agent(
                self.query, self.cfg, self.parent_query
            )
    ...
    async def get_context_by_search(self, query):
        ...
        context = await asyncio.gather(
            *[self.process_sub_query(sub_query) for sub_query in sub_queries]
        )
        return context
    ...
    async def scrape_sites_by_query(self, sub_query):
        ...
        await stream_output(
            "logs", "🤔 Researching for relevant information...\n", self.websocket
        )
    ...
    async def get_subtopics(self):
        ...
        """
        await stream_output("logs", "🤔 Generating subtopics...", self.websocket)
